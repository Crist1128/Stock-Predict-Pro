...
for i in range(predict_minutes):
    ...
    # 反归一化得到实际股价
    predicted_price = scaler.inverse_transform(y.detach().numpy())
    
    # 获取当前时间并转为字符串格式
    current_time = (end_date + timedelta(minutes=i)).strftime('%Y-%m-%d %H:%M:%S')
    
    # 存储预测结果到字典
    predicted_dict[current_time] = predicted_price[0, 0]
